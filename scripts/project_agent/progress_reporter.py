...
def issue_lines(items: list[dict[str, object]], empty: str) -> str:
    if not items:
        return empty
    return "\n".join(f"- #{item['number']} {item['title']}" for item in items)


def milestone_lines(items: list[dict[str, object]]) -> str:
    if not items:
...
def dependency_blocker_lines(items: list[dict[str, object]]) -> str:
    if not items:
        return "- None"
    return "\n".join(
...
def deterministic_report(context: dict[str, object]) -> str:
...
### Closed This Week
{issue_lines(context['closed_this_week'], '- None')}

### Opened This Week
{issue_lines(context['opened_this_week'], '- None')}

### In Progress
{issue_lines(context['in_progress'], '- None')}

### Blockers
{issue_lines(context['currently_blocked'], '- None')}

### Milestone Progress
{milestone_lines(context['milestones'])}

### Dependency Blockers
{dependency_blocker_lines(context['dependency_blockers'])}
"""
...
def main() -> None:
    ...
    try:
        all_closed = retry(lambda: list(repo.get_issues(state="closed")))
        all_open = retry(lambda: list(repo.get_issues(state="open")))
        all_milestones = retry(lambda: list(repo.get_milestones(state="open")))
    except Exception as exc:
        log_event("progress_reporter_data_fetch_failed", error=str(exc))
        raise
        ...
        response = retry(
            lambda: ai.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": f"Generate a narrative from this data:\n{json.dumps(context, indent=2)}",
                    },
                ],
                timeout=60,
            )
    ...
    report_body = (
        deterministic_report(context)
        + "\n\n"
        + ai_narrative
        + "\n\n---\n"
        + agent_signature("progress_reporter", context="weekly progress reporting")
        + "\n"
        + "> Source data: GitHub issues and milestones API."
    )

    title = f"📊 Weekly Progress Report — Week of {now.strftime('%Y-%m-%d')}"
    available_labels = [label.name for label in retry(lambda: list(repo.get_labels()))]
    labels_to_apply = [label for label in REQUIRED_LABELS if label in available_labels]

    if dry_run:
        print(f"[dry-run] Would create weekly report issue: {title}")
        log_event("progress_report_dry_run", title=title, week_ending=now.strftime("%Y-%m-%d"))
        return
